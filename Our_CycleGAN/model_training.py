...
class Trainer():
    ...
    def train_step(self, real_A, real_B):
        ...
        # ---------------------------------------------------
        loss_G_total, loss_G_A2B, loss_G_B2A, loss_cycle, loss_identity, fake_A, fake_B = self.gan_training_step(real_A, real_B, real_target)
    ...
    def gan_training_step(self, real_A, real_B, real_target):
...
class Trainer_CBAM_GL_V2(Trainer):
    ...
    def gan_training_step(self, real_A, real_B, real_target, fake_target):
        ...
        # G_A2B(A) should look like B (Normal)
        fake_B = self.model.G_A2B(real_A)
        fake_B_locals = self.sample_local_patch(fake_B)
        pred_fake_B = self.model.D_B(fake_B, fake_B_locals)
        loss_G_A2B = self.criterion_GAN(pred_fake_B, real_target)

        # G_B2A(B) should look like A (Low)
        fake_A = self.model.G_B2A(real_B)
        fake_A_locals = self.sample_local_patch(fake_A)
        pred_fake_A = self.model.D_A(fake_A, fake_A_locals)
        loss_G_B2A = self.criterion_GAN(pred_fake_A, real_target)
    ...
    def discriminator_training_step(self, real_A, real_A_patches, real_B, real_B_patches, real_target, fake_target, fake_A, fake_B):
        ...
        fake_B_buffered = self.fake_B_buffer.push_and_pop(fake_B)
        # pred_fake_B_buff = self.model.D_B(fake_B_buffered.detach()) # Detach is crucial here!
        fake_B_buffered_locals = self.sample_local_patch(fake_B_buffered)
        pred_fake_B_buff = self.model.D_B(fake_B_buffered.detach(), fake_B_buffered_locals.detach()) # Detach is crucial here!
        loss_D_B_fake = self.criterion_GAN(pred_fake_B_buff, fake_target)
        ...
        fake_A_buffered = self.fake_A_buffer.push_and_pop(fake_A)
        # pred_fake_A_buff = self.model.D_A(fake_A_buffered.detach()) # Detach is crucial here!
        fake_A_buffered_locals = self.sample_local_patch(fake_A_buffered)
        pred_fake_A_buff = self.model.D_A(fake_A_buffered.detach(), fake_A_buffered_locals.detach()) # Detach is crucial here!
        loss_D_A_fake = self.criterion_GAN(pred_fake_A_buff, fake_target)
